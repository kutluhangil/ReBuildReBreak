...
const App: React.FC = () => {
  ...
  const handleSaveBuild = (id: string, newName: string, newFolder?: string) => {
    setCustomBuilds((prev) =>
      prev.map((b) =>
        b.id === id ? { ...b, name: newName, folder: newFolder } : b,
      ),
    );
    setCustomRebuilds((prev) =>
      prev.map((b) =>
        b.id === id ? { ...b, name: newName, folder: newFolder } : b,
      ),
    );
  ...
  useEffect(() => {
    localStorage.setItem(
      "voxel_architect_rebuilds",
      JSON.stringify(customRebuilds),
    );
  }, [customRebuilds]);
  ...
  useEffect(() => {
    ...
    // audio initialization removed (silent build)
  ...
  useEffect(() => {
    if (engineRef.current) {
      ...
      engineRef.current.currentColor = parseInt(
  ...
  useEffect(() => {
    if (engineRef.current) {
      if (
        (engineRef.current as any).gridSnapping !== undefined &&
        (engineRef.current as any).gridSnapping !== gridSnapping
      ) {
  ...
  const [environment, setEnvironment] = useState<"day" | "night" | "sunset">(
    "day",
  );
  ...
  const handleUndo = () => {
    if (historyIndex > 0 && engineRef.current) {
      ...
      showToast(
        `Undo: ${history[historyIndex].actionName || "Action reverted"}`,
      );
    }
  };
  ...
  const handleClear = () => {
    if (
      window.confirm(
        "Are you sure you want to delete all voxels from the scene? This cannot be undone.",
      )
    ) {
      if (engineRef.current) {
    ...
    <div className="relative w-full h-screen bg-[#f0f2f5] overflow-hidden">
      ...
      <UIOverlay
        ...
        onToggleCameraProjection={() =>
          engineRef.current?.toggleCameraProjection()
        }
      ...
      <div
        className={`fixed bottom-4 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 bg-slate-800 text-white font-bold rounded-full shadow-2xl transition-all duration-300 pointer-events-none ${toastMessage ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
      >
        {toastMessage}
      </div>
    </div>
