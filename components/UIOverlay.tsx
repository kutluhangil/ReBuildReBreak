...
interface UIOverlayProps {
  ...
  onSymmetryChange: (symmetry: { x: boolean; y: boolean; z: boolean }) => void;
  onShowHelp: () => void;
}
...
export const UIOverlay: React.FC<UIOverlayProps> = ({
  ...
  const buildsByFolder = customBuilds.reduce(
    (acc, build) => {
  ...
  const rebuildsByFolder = customRebuilds.reduce(
    (acc, build) => {
      const folder = build.folder || "Uncategorized";
      if (!acc[folder]) acc[folder] = [];
      acc[folder].push(build);
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-start">
        ...
        <div className="pointer-events-auto flex flex-col gap-2">
          ...
          >
            ...
            <DropdownItem
              onClick={() => onNewScene("Eagle")}
              icon={<Bird size={16} />}
              label="Eagle"
            />
            <DropdownItem
              onClick={() => onNewScene("Cat")}
              icon={<Cat size={16} />}
              label="Cat"
            />
            <DropdownItem
              onClick={() => onNewScene("Terrain")}
              icon={<Trees size={16} />}
              label="Terrain"
            />
            <DropdownItem
              onClick={() => onNewScene("Rabbit")}
              icon={<Rabbit size={16} />}
              label="Rabbit"
            />
            <DropdownItem
              onClick={() => onNewScene("Twins")}
              icon={<Users size={16} />}
              label="Twins"
            />
            <DropdownItem
              onClick={() => onNewScene("Robot")}
              icon={<Bot size={16} />}
              label="Robot"
            />
            <DropdownItem
              onClick={() => onNewScene("House")}
              icon={<Home size={16} />}
              label="House"
            />
            <DropdownItem
            ...
            <DropdownItem
              onClick={() => onSaveBuild("Custom Build")}
              icon={<Plus size={16} />}
          ...
          <div className="flex items-center gap-3 px-4 py-2 bg-white/90 backdrop-blur-sm shadow-sm rounded-xl border border-slate-200 text-slate-500 font-bold w-fit mt-2">
            ...
            <div className="flex flex-col leading-none">
              ...
              <span className="text-lg text-slate-800 font-extrabold font-mono">
                {voxelCount}
              </span>
            </div>
          </div>
        ...
        <div className="pointer-events-auto flex gap-2">
          ...
          {customBuilds.length > 0 && (
            <div className="relative group">
              <TactileButton
                onClick={() =>
                  setShowRecentModelsDropdown(!showRecentModelsDropdown)
                }
                color="slate"
                icon={<History as HistoryIcon size={18} strokeWidth={2.5} />}
                label="Recent"
              />
              {showRecentModelsDropdown && (
                <div className="absolute top-full mt-1 right-0 bg-white/95 backdrop-blur-sm rounded-lg shadow-xl border border-slate-200 z-50 w-48 max-h-64 overflow-y-auto">
                  {customBuilds.slice(0, 5).map((build) => (
                    <button
                      key={build.id}
                      onClick={() => {
                        onSelectCustomBuild(build);
                        setShowRecentModelsDropdown(false);
                      }}
                      className="w-full text-left px-3 py-2 hover:bg-indigo-50 text-sm text-slate-700 font-medium border-b border-slate-100 last:border-b-0"
                    >
                      {build.name}
                    </button>
                  ))}
      ...
      {isStable && (
        <div className="absolute top-1/2 left-4 -translate-y-1/2 pointer-events-auto flex flex-col gap-4">
          ...
          <div className="bg-white/90 backdrop-blur p-2 rounded-2xl shadow-xl flex flex-col gap-2 border-2 border-slate-200">
            <ToolButton
              icon={<Plus size={20} />}
              active={currentTool === BrushTool.ADD}
              onClick={() => onToolChange(BrushTool.ADD)}
              tooltip="Add Voxel"
            />
            <ToolButton
              icon={<Eraser size={20} />}
              active={currentTool === BrushTool.REMOVE}
              onClick={() => onToolChange(BrushTool.REMOVE)}
              tooltip="Remove Voxel"
            />
            <ToolButton
              icon={<Paintbrush size={20} />}
              active={currentTool === BrushTool.PAINT}
              onClick={() => onToolChange(BrushTool.PAINT)}
              tooltip="Paint Voxel"
            />
            <ToolButton
              icon={<Fingerprint size={20} />}
              active={currentTool === BrushTool.SCULPT}
              onClick={() => onToolChange(BrushTool.SCULPT)}
              tooltip="Sculpt Voxel"
            />
          </div>
          ...
          {currentTool === BrushTool.SCULPT && (
            <div className="bg-white/90 backdrop-blur p-4 rounded-2xl shadow-xl flex flex-col gap-4 border-2 border-slate-200 w-40 mt-2">
              <div className="flex flex-col gap-1 group relative">
                <div
                  className="flex justify-between text-xs font-bold text-slate-500 uppercase cursor-help"
                  title="Controls the radius of the sculpting brush. A larger size affects more blocks at once."
                >
                  <span>Size</span>
              ...
              <div className="flex flex-col gap-1 group relative">
                <div
                  className="flex justify-between text-xs font-bold text-slate-500 uppercase cursor-help"
                  title="Controls the intensity of the sculpt effect. Higher strength builds or digs faster."
                >
                  <span>Strength</span>
          ...
          <div className="bg-white/90 backdrop-blur p-4 rounded-2xl shadow-xl flex flex-col gap-4 border-2 border-slate-200 w-40 mt-2">
            <div className="flex flex-col gap-1">
              <div className="flex justify-between text-xs font-bold text-slate-500 uppercase">
                ...
                <span>{brushSize.toFixed(1)}x</span>
              </div>
              <input
                type="range"
                min="1"
              ...
              <div className="grid grid-cols-3 gap-1">
                {(["x", "y", "z"] as const).map((axis) => (
                  <button
                    ...
                    className={`py-1 px-2 rounded font-bold text-xs uppercase transition-all ${
                      symmetry[axis]
            ...
            currentTool !== BrushTool.SCULPT && (
              <div className="bg-white/90 backdrop-blur p-2 rounded-2xl shadow-xl flex flex-col gap-2 border-2 border-slate-200 mt-2 max-h-56 overflow-y-auto w-36">
                <MaterialButton
                  ...
                  active={currentMaterial === MaterialType.SOLID}
                  onClick={() => onMaterialChange(MaterialType.SOLID)}
                  label="Solid"
                />
                <MaterialButton
                  type={MaterialType.METAL}
                  active={currentMaterial === MaterialType.METAL}
                  onClick={() => onMaterialChange(MaterialType.METAL)}
                  label="Metal"
                />
                <MaterialButton
                  type={MaterialType.WOOD}
                  active={currentMaterial === MaterialType.WOOD}
                  onClick={() => onMaterialChange(MaterialType.WOOD)}
                  label="Wood"
                />
                <MaterialButton
                  type={MaterialType.GLASS}
                  active={currentMaterial === MaterialType.GLASS}
                  onClick={() => onMaterialChange(MaterialType.GLASS)}
                  label="Glass"
                />
                <MaterialButton
                  type={MaterialType.STONE}
                  active={currentMaterial === MaterialType.STONE}
                  onClick={() => onMaterialChange(MaterialType.STONE)}
                  label="Stone"
                />
                <MaterialButton
                  type={MaterialType.PLASTIC}
                  active={currentMaterial === MaterialType.PLASTIC}
                  onClick={() => onMaterialChange(MaterialType.PLASTIC)}
                  label="Plastic"
                />
                <MaterialButton
                  type={MaterialType.FABRIC}
                  active={currentMaterial === MaterialType.FABRIC}
                  onClick={() => onMaterialChange(MaterialType.FABRIC)}
                  label="Fabric"
                />
              </div>
            )}
          ...
          {hoveredVoxel &&
            (currentTool === BrushTool.PAINT ||
              ...
              <div className="bg-white/95 backdrop-blur p-3 rounded-2xl shadow-xl flex flex-col gap-1 border-2 border-slate-200 mt-2 text-xs font-mono text-slate-700 w-32">
                <div className="font-bold text-slate-400 uppercase tracking-wider mb-1 border-b border-slate-100 pb-1">
                  Voxel Info
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">X:</span> {hoveredVoxel.x}
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Y:</span> {hoveredVoxel.y}
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Z:</span> {hoveredVoxel.z}
                </div>
                <div
                  className="mt-1 pt-1 border-t border-slate-100 font-bold capitalize text-indigo-500 truncate"
                  title={hoveredVoxel.material}
                >
                  {hoveredVoxel.material}
                </div>
              </div>
            )}
        </div>
        ...
        <div className="absolute top-20 right-4 w-64 bg-white/95 backdrop-blur-md border-2 border-slate-200 p-4 rounded-3xl shadow-2xl pointer-events-auto">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1">
              <div className="flex justify-between text-xs font-bold text-slate-500 uppercase">
                <span>Gravity</span>
                <span>{physicsConfig.gravity.toFixed(1)}</span>
              </div>
              <input
                type="range"
                min="-30"
            ...
            <div className="flex flex-col gap-1">
              <div className="flex justify-between text-xs font-bold text-slate-500 uppercase">
                <span>Bounce</span>
                <span>{physicsConfig.bounce.toFixed(2)}</span>
              </div>
              <input
                type="range"
                min="0"
            ...
            <div className="flex flex-col gap-1">
              <div className="flex justify-between text-xs font-bold text-slate-500 uppercase">
                <span>Friction</span>
                <span>{physicsConfig.friction.toFixed(2)}</span>
              </div>
              <input
                type="range"
                min="0"
            ...
            <div className="flex flex-col gap-1">
              <div className="flex justify-between text-xs font-bold text-slate-500 uppercase">
                <span>Explosion</span>
                <span>{physicsConfig.explosionForce.toFixed(1)}</span>
              </div>
              <input
                type="range"
                min="0"
                max="5"
                step="0.1"
                value={physicsConfig.explosionForce}
                onChange={(e) =>
                  onPhysicsConfigChange({
                    ...physicsConfig,
      ...
      {showMaterialPanel && (
        <div className="absolute top-20 right-4 w-72 bg-white/95 backdrop-blur-md border-2 border-slate-200 p-4 rounded-3xl shadow-2xl pointer-events-auto flex flex-col gap-4">
          ...
          <div className="text-center">
            <span className="text-sm font-bold text-slate-700 bg-slate-100 px-3 py-1 rounded-full uppercase tracking-wider">
              {currentMaterial}
            </span>
          </div>
          ...
          {(() => {
            ...
            if (currentMaterial === MaterialType.METAL) {
              presets.push({
                label: "Brushed",
                config: { roughness: 0.4, metalness: 0.9 },
              });
              presets.push({
                label: "Polished",
                config: { roughness: 0.1, metalness: 1.0 },
              });
              presets.push({
                label: "Rusty",
                config: { roughness: 0.8, metalness: 0.6 },
              });
            } else if (currentMaterial === MaterialType.GLASS) {
              presets.push({
                label: "Clear",
                config: { roughness: 0.05, metalness: 0.1, transmission: 0.95 },
              });
              presets.push({
                label: "Frosted",
                config: { roughness: 0.4, metalness: 0.1, transmission: 0.8 },
              });
              presets.push({
                label: "Tinted",
                config: { roughness: 0.1, metalness: 0.3, transmission: 0.6 },
              });
            } else if (currentMaterial === MaterialType.STONE) {
              presets.push({
                label: "Rough Concrete",
                config: { roughness: 0.95, metalness: 0.05 },
              });
              presets.push({
                label: "Polished Marble",
                config: { roughness: 0.1, metalness: 0.1 },
              });
            } else if (currentMaterial === MaterialType.PLASTIC) {
              presets.push({
                label: "Shiny",
                config: { roughness: 0.2, metalness: 0.0, clearcoat: 1.0 },
              });
              presets.push({
                label: "Matte",
                config: { roughness: 0.7, metalness: 0.0, clearcoat: 0.1 },
              });
            } else if (currentMaterial === MaterialType.WOOD) {
              presets.push({
                label: "Varnished",
                config: { roughness: 0.3, metalness: 0.0 },
              });
              presets.push({
                label: "Raw",
                config: { roughness: 0.85, metalness: 0.0 },
              });
            }
                  ...
                  <button
                    ...
                    onClick={() =>
                      onMaterialConfigChange({
                        ...materialConfig,
                        [currentMaterial]: {
                          ...materialConfig[currentMaterial],
                          ...p.config,
                        },
                      })
                    }
          ...
          <div className="flex flex-col gap-2 mt-2">
            <details className="group border border-slate-200 rounded-xl overflow-hidden [&_summary::-webkit-details-marker]:hidden bg-slate-50">
              <summary className="flex items-center justify-between cursor-pointer p-3 font-bold text-sm text-slate-700 hover:bg-slate-100 transition-colors">
                <span title="Base surface parameters that affect how light scatters">
                  Core Properties
                </span>
              ...
              <div className="p-3 pt-1 flex flex-col gap-4 bg-white border-t border-slate-100">
                <div
                  ...
                  title="Roughness determines how scattered light gets when bouncing off the surface. 0 means perfectly smooth like a mirror, 1 means completely rough like chalk."
                >
                  <div className="flex justify-between text-xs font-bold text-slate-500 uppercase">
                    <span>Roughness</span>
                    <span>
                      {(
                        materialConfig[currentMaterial]?.roughness ?? 0.8
                      ).toFixed(2)}
                    </span>
                  </div>
                  <input
                    ...
                    max="1"
                    step="0.05"
                    value={materialConfig[currentMaterial]?.roughness ?? 0.8}
                    onChange={(e) =>
                      onMaterialConfigChange({
                        ...materialConfig,
                        [currentMaterial]: {
                ...
                <div
                  className="flex flex-col gap-1"
                  title="Metalness makes a material look more like a metal rather than a dielectric material like plastic or wood."
                >
                  <div className="flex justify-between text-xs font-bold text-slate-500 uppercase">
                    <span>Metalness</span>
                    <span>
                      {(
                        materialConfig[currentMaterial]?.metalness ?? 0.1
                      ).toFixed(2)}
                    </span>
                  </div>
                  <input
                    ...
                    max="1"
                    step="0.05"
                    value={materialConfig[currentMaterial]?.metalness ?? 0.1}
                    onChange={(e) =>
                      onMaterialConfigChange({
                        ...materialConfig,
                        [currentMaterial]: {
            ...
            {(currentMaterial === MaterialType.GLASS ||
              ...
              <details className="group border border-slate-200 rounded-xl overflow-hidden [&_summary::-webkit-details-marker]:hidden bg-slate-50">
                <summary className="flex items-center justify-between cursor-pointer p-3 font-bold text-sm text-slate-700 hover:bg-slate-100 transition-colors">
                  <span title="Advanced parameters specific to this material type">
                    Advanced Properties
                  </span>
                  <span className="transition group-open:rotate-180">
                    <ChevronUp size={16} />
                ...
                <div className="p-3 pt-1 flex flex-col gap-4 bg-white border-t border-slate-100">
                  {currentMaterial === MaterialType.GLASS && (
                    <div
                      className="flex flex-col gap-1"
                      title="Transmission controls how much light passes through the surface, giving the effect of glass."
                    >
                      <div className="flex justify-between text-xs font-bold text-slate-500 uppercase">
                        <span>Transmission</span>
                        <span>
                          {(
                            materialConfig[currentMaterial]?.transmission ?? 0.9
                          ).toFixed(2)}
                        </span>
                      </div>
                      <input
                  ...
                  {currentMaterial === MaterialType.PLASTIC && (
                    <div
                      className="flex flex-col gap-1"
                      title="Clearcoat adds a transparent reflective layer on top of the base material, similar to car paint."
                    >
                      <div className="flex justify-between text-xs font-bold text-slate-500 uppercase">
                        <span>Clearcoat</span>
                        <span>
                          {(
                            materialConfig[currentMaterial]?.clearcoat ?? 1.0
                          ).toFixed(2)}
                        </span>
                      </div>
                      <input
                  ...
                  {currentMaterial === MaterialType.FABRIC && (
                    <div
                      className="flex flex-col gap-1"
                      title="Sheen adds a soft reflection at grazing angles, characteristic of cloth and velvet materials."
                    >
                      <div className="flex justify-between text-xs font-bold text-slate-500 uppercase">
                        <span>Sheen</span>
                        <span>
                          {(
                            materialConfig[currentMaterial]?.sheen ?? 1.0
                          ).toFixed(2)}
                        </span>
                      </div>
                      <input
                        ...
                        max="1"
                        step="0.05"
                        value={materialConfig[currentMaterial]?.sheen ?? 1.0}
                        onChange={(e) =>
                          onMaterialConfigChange({
                            ...materialConfig,
                            [currentMaterial]: {
          ...
          {isDismantling && !isGenerating && (
            <div className="flex items-end gap-4 animate-in slide-in-from-bottom-10 fade-in duration-300">
              ...
              >
                ...
                <DropdownItem
                  onClick={() => onRebuild("Eagle")}
                  icon={<Bird size={18} />}
                  label="Eagle"
                />
                <DropdownItem
                  onClick={() => onRebuild("Cat")}
                  icon={<Cat size={18} />}
                  label="Cat"
                />
                <DropdownItem
                  onClick={() => onRebuild("Rabbit")}
                  icon={<Rabbit size={18} />}
                  label="Rabbit"
                />
                <DropdownItem
                  onClick={() => onRebuild("Twins")}
                  icon={<Users size={18} />}
                  label="Eagles x2"
                />
                <DropdownItem
                  onClick={() => onRebuild("Tree")}
                  icon={<Trees size={18} />}
                  label="Tree"
                />
                <DropdownItem
                  onClick={() => onRebuild("House")}
                  icon={<Home size={18} />}
                  label="House"
                />
                <DropdownItem
                  onClick={() => onRebuild("Robot")}
                  icon={<Bot size={18} />}
                  label="Robot"
                />
                <div className="h-px bg-slate-100 my-1" />
  ...
  const getPreview = () => {
    switch (type) {
      case MaterialType.SOLID:
        return (
          <div className="w-5 h-5 rounded-md bg-slate-500 shadow-sm border border-slate-600" />
        );
      case MaterialType.METAL:
        return (
          <div className="w-5 h-5 rounded-md bg-gradient-to-br from-slate-200 via-slate-400 to-slate-600 shadow-sm border border-slate-400" />
        );
      case MaterialType.WOOD:
        return (
          <div className="w-5 h-5 rounded-md bg-amber-700 shadow-sm border border-amber-900 bg-[repeating-linear-gradient(45deg,transparent,transparent_2px,rgba(0,0,0,0.1)_2px,rgba(0,0,0,0.1)_4px)]" />
        );
      case MaterialType.GLASS:
        return (
          <div className="w-5 h-5 rounded-md bg-sky-200/40 shadow-sm border-2 border-sky-300/50 backdrop-blur-sm" />
        );
      case MaterialType.STONE:
        return (
          <div className="w-5 h-5 rounded-md bg-stone-500 shadow-sm border border-stone-600 bg-[radial-gradient(circle_at_center,rgba(0,0,0,0.1)_1px,transparent_1px)] bg-[size:4px_4px]" />
        );
      case MaterialType.PLASTIC:
        return (
          <div className="w-5 h-5 rounded-md bg-pink-400 shadow-sm border border-pink-500 saturate-150" />
        );
      case MaterialType.FABRIC:
        return (
          <div className="w-5 h-5 rounded-md bg-indigo-400 shadow-sm border border-indigo-500 bg-[repeating-linear-gradient(0deg,transparent,transparent_2px,rgba(255,255,255,0.2)_2px,rgba(255,255,255,0.2)_4px)]" />
        );
      default:
...
// Added wrapper for bottom hints
const ShortcutsHint = () => (
  <div className="fixed bottom-4 left-4 text-[10px] font-mono text-slate-400 bg-white/50 backdrop-blur-md px-3 py-2 rounded-lg shadow-sm border border-white/40 z-40 hidden md:flex gap-4 pointer-events-none">
    <span>
      <kbd className="bg-slate-100 border border-slate-200 px-1 rounded text-slate-500 font-sans">
        1-4
      </kbd>{" "}
      Tools
    </span>
...
const TactileButton: React.FC<TactileButtonProps> = ({
  ...
  const colorStyles = {
    ...
    emerald:
      "bg-emerald-500 text-white shadow-emerald-700 hover:bg-emerald-600",
...
const DropdownMenu: React.FC<DropdownProps> = ({
  ...
  const bgClass =
    color === "indigo"
      ? "bg-indigo-500 hover:bg-indigo-600 border-indigo-800"
      : "bg-emerald-500 hover:bg-emerald-600 border-emerald-800";
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        ...
        <ChevronUp
          size={16}
          className={`transition-transform duration-300 ${isOpen ? "rotate-180" : ""} ${direction === "down" ? "rotate-180" : ""}`}
        />
      </button>
...
const DropdownItem: React.FC<{
  ...
  highlight?: boolean;
  truncate?: boolean;
}> = ({ onClick, onEdit, icon, label, highlight, truncate }) => {
  return (
    <div className="relative group">
      <button
        onClick={onClick}
      ...
      >
        ...
        <span
          className={
            truncate
              ? "truncate w-[80%] block overflow-hidden text-ellipsis whitespace-nowrap"
              : "w-full block"
          }
        >
          {label}
